...
export function renderHeader() {
    ...
    let notificationsListHTML = state.notifications.length > 0 
        ? state.notifications.map(notif => {
            ...
            if (['balance_alert', 'budget_alert', 'installment_alert', 'installment_due_today', 'goal_alert', 'inactivity_alert'].includes(notif.type)) {
                 if (notif.type === 'balance_alert') { bgClass = 'bg-red-50 dark:bg-red-900/20'; borderLeftColor = 'border-l-red-500'; icon = '⚠️'; }
                 // ... (outros ifs de sistema mantidos igual ao seu código atual) ...
                 else if (notif.type === 'budget_alert') { bgClass = 'bg-amber-50 dark:bg-amber-900/20'; borderLeftColor = 'border-l-amber-500'; icon = '📉'; }
                 else if (notif.type === 'installment_alert') { bgClass = 'bg-blue-50 dark:bg-blue-900/20'; borderLeftColor = 'border-l-blue-500'; icon = '📅'; }
                 else if (notif.type === 'installment_due_today') { bgClass = 'bg-orange-50 dark:bg-orange-900/20'; borderLeftColor = 'border-l-orange-500'; icon = '⏰'; }
                 else if (notif.type === 'goal_alert') { bgClass = 'bg-green-50 dark:bg-green-900/20'; borderLeftColor = 'border-l-green-500'; icon = '🏆'; }
                 else { bgClass = 'bg-gray-50 dark:bg-gray-800'; borderLeftColor = 'border-l-gray-400'; icon = 'zzz'; }
            }
            
            // 2. Convites e Respostas
            else if (['join_request', 'request_accepted', 'request_rejected'].includes(notif.type)) {
                borderLeftColor = 'border-l-brand-500';
                icon = '👋';
                if (notif.type === 'request_rejected') {
                    borderLeftColor = 'border-l-red-500'; // Borda vermelha para rejeição
                    icon = '🚫'; // Ícone de bloqueio
                }
            }

            const finalBg = !notif.read ? bgClass.replace('bg-', 'bg-opacity-100 bg-') : bgClass;
            
            // CONTEÚDO DA MENSAGEM (CORREÇÃO AQUI)
            let messageHTML = '';
            ...
            if (notif.title) {
                ...
                if (notif.type === 'join_request') {
                    text = `<strong>${notif.senderName}</strong> quer entrar na família <strong>${notif.targetFamilyName || ''}</strong>.`;
                } else if (notif.type === 'request_accepted') {
                    text = `<strong>${notif.senderName || 'Admin'}</strong> aceitou seu convite para <strong>${notif.targetFamilyName || ''}</strong>.`;
                } else if (notif.type === 'request_rejected') {
                    // CORREÇÃO DO TEXTO DE REJEIÇÃO
                    text = `<strong>${notif.senderName || 'Admin'}</strong> recusou sua solicitação para entrar em <strong>${notif.targetFamilyName || 'uma família'}</strong>.`;
                }
