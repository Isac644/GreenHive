...
export function renderHeader() {
  ...
  const hasUnread =
    state.notifications && state.notifications.some((n) => !n.read);
  const badgeHTML = hasUnread
    ? `<span class="absolute top-2 right-2 block h-2.5 w-2.5 rounded-full ring-2 ring-white dark:ring-gray-900 bg-brand-500 animate-pulse"></span>`
    : "";
    ...
    state.notifications.length > 0
      ? state.notifications
          .map((notif) => {
            ...
            // 1. Alertas de Sistema (Saldo, Orçamento, Parcela)
            if (
              [
                "balance_alert",
                "budget_alert",
            ...
            // 2. Convites e Respostas
            else if (
              ["join_request", "request_accepted", "request_rejected"].includes(
                notif.type
              )
            ) {
            ...
            const finalBg = !notif.read
              ? bgClass.replace("bg-", "bg-opacity-100 bg-")
              : bgClass;

            let messageHTML = "";

            if (notif.title) {
              // Notificação com Título (Alertas de Sistema)
              messageHTML = `
                    <div>
              ...
              // Notificações Sociais (Convites)
              let text = "";
              if (notif.type === "join_request") {
                text = `<strong>${
                  notif.senderName
                }</strong> quer entrar na família <strong>${
                  notif.targetFamilyName || ""
                }</strong>.`;
              } else if (notif.type === "request_accepted") {
                text = `<strong>${
                  notif.senderName || "Admin"
                }</strong> aceitou seu convite para <strong>${
                  notif.targetFamilyName || ""
                }</strong>.`;
              } else if (notif.type === "request_rejected") {
                text = `<strong>${
                  notif.senderName || "Admin"
                }</strong> recusou sua solicitação para entrar em <strong>${
                  notif.targetFamilyName || "uma família"
                }</strong>.`;
              }

              messageHTML = `<p class="text-sm text-gray-600 dark:text-gray-300 leading-snug">${text}</p>`;
            }

            const content = `<div class="flex gap-3 pr-6"><span class="text-xl mt-1">${icon}</span>${messageHTML}</div>`;
  ...
  let userAvatar = `<div class="w-10 h-10 rounded-full bg-brand-100 dark:bg-brand-900 text-brand-600 dark:text-brand-300 flex items-center justify-center font-heading font-bold text-lg shadow-sm ring-2 ring-white dark:ring-gray-800 select-none">${state.user.name
    .charAt(0)
    .toUpperCase()}</div>`;
  ...
  return `<header class="sticky top-0 z-40 w-full bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg border-b border-gray-200/50 dark:border-gray-800 transition-colors duration-300"><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div class="flex justify-between items-center h-20"><div class="flex items-center gap-3 hover:opacity-80 transition cursor-default">${GreenHiveLogoSVG(
    ...
    Icons.Bell
  }${badgeHTML}</button>${dropdownHTML}</div><button id="theme-toggle-button" class="p-2.5 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500 dark:text-gray-400 transition-all active:scale-95 focus:outline-none">${
    state.theme === "light" ? Icons.Moon : Icons.Sun
  }</button><div class="relative pl-2"><button id="user-menu-button" class="flex items-center gap-2 transition-transform active:scale-95 focus:outline-none">${userAvatar}</button><div id="user-menu" class="hidden absolute right-0 mt-4 w-64 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 py-2 animate-fade-in origin-top-right z-50 ring-1 ring-black/5"><div class="px-5 py-3 border-b border-gray-100 dark:border-gray-700 mb-2"><p class="text-sm font-heading font-bold text-gray-800 dark:text-white truncate">${
    state.user.name
...
export function renderAuthPage() {
  const content =
    state.authView === "login"
      ...
      ? renderForgotPasswordSuccessHTML()
      : renderSignupSuccessHTML();
  ...
  return `<div class="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl p-8 sm:p-12 rounded-3xl shadow-2xl w-full max-w-md mx-auto border border-white/50 dark:border-gray-700 relative z-10 animate-fade-in">
            <div class="flex flex-col items-center mb-8 text-center">
                <div class="mb-4 transform scale-125">${GreenHiveLogoSVG(
                  "48"
                )}</div>
    ...
    <div class="min-h-screen flex items-center justify-center p-4 bg-gray-50 dark:bg-gray-900 relative overflow-hidden">
         ...
         <div class="w-full max-w-4xl relative z-10 flex flex-col justify-center min-h-[80vh] sm:min-h-0">
            ...
            <div class="text-center mb-8 sm:mb-12">
                <h1 class="text-3xl sm:text-5xl font-heading font-bold text-gray-900 dark:text-white mb-2 sm:mb-4">Olá, ${
        ...
        Entrar
                    ...
                    ${
                      state.joinRequestMessage
                        ? `<p class="mt-3 text-xs font-bold text-amber-600 text-center bg-amber-50 dark:bg-amber-900/20 p-2 rounded-lg animate-fade-in">${state.joinRequestMessage}</p>`
                        : ""
                    }
                </div>
            </div>
            ...
            <div class="mt-8 sm:mt-12">
                ...
                <div class="flex flex-wrap justify-center gap-3 sm:gap-4">
                    ...
                    <button data-family-id="${
                      f.id
                    }" class="select-family-button flex items-center gap-3 px-5 py-3 bg-white dark:bg-gray-800 rounded-2xl shadow-sm hover:shadow-md hover:-translate-y-1 transition-all border border-gray-100 dark:border-gray-700 group w-full sm:w-auto justify-center">
                        <div class="w-8 h-8 rounded-full bg-brand-100 dark:bg-brand-900/50 text-brand-600 flex items-center justify-center font-bold text-sm">${f.name.charAt(
                          0
                        )}</div>
                        <span class="font-heading font-bold text-gray-800 dark:text-white text-base group-hover:text-brand-600 transition-colors">${
                          f.name
                        }</span>
...
export function renderMainContent() {
  ...
  const activeClass =
    "bg-brand-100 text-brand-700 dark:bg-brand-900/30 dark:text-brand-300 shadow-sm ring-1 ring-brand-200 dark:ring-brand-800";
  const inactiveClass =
    "text-gray-500 hover:text-gray-700 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800/50 transition-colors";
  ...
  const navTabs = `
    <div class="mb-8 overflow-x-auto pb-2 no-scrollbar">
        <nav class="flex space-x-2 p-1.5 bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm rounded-2xl border border-gray-200/50 dark:border-gray-700/50 w-max mx-auto sm:w-full sm:mx-0">
            <button data-view="dashboard" class="nav-tab ${
              state.currentView === "dashboard" ? activeClass : inactiveClass
            } flex-1 py-2.5 px-6 rounded-xl text-sm font-heading font-bold transition-all duration-200 whitespace-nowrap">Dashboard</button>
            <button data-view="records" class="nav-tab ${
              state.currentView === "records" ? activeClass : inactiveClass
            } flex-1 py-2.5 px-6 rounded-xl text-sm font-heading font-bold transition-all duration-200 whitespace-nowrap">Registros</button>
            <button data-view="budget" class="nav-tab ${
              state.currentView === "budget" ? activeClass : inactiveClass
            } flex-1 py-2.5 px-6 rounded-xl text-sm font-heading font-bold transition-all duration-200 whitespace-nowrap">Orçamento</button>
            <button data-view="debts" class="nav-tab ${
              state.currentView === "debts" ? activeClass : inactiveClass
            } flex-1 py-2.5 px-6 rounded-xl text-sm font-heading font-bold transition-all duration-200 whitespace-nowrap">Dívidas</button>
            <button data-view="goals" class="nav-tab ${
              state.currentView === "goals" ? activeClass : inactiveClass
            } flex-1 py-2.5 px-6 rounded-xl text-sm font-heading font-bold transition-all duration-200 whitespace-nowrap">Metas</button>
        </nav>
    </div>`;
    ...
    <div class="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative">
        <div class="flex flex-row justify-between items-center mb-8 gap-4">
            <div class="flex-1 min-w-0">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1 font-medium truncate">Bem-vindo de volta, ${
                  state.user.name.split(" ")[0]
                }! 👋</p>
            </div>
...
export function renderFamilyDashboard() {
  ...
  const monthName = state.displayedMonth.toLocaleString("pt-BR", {
    month: "long",
  });

  const monthlyTransactions = state.transactions.filter(
    (t) =>
      new Date(t.date + "T12:00:00").getMonth() === month &&
  ...
  const userSummary = userTransactions.reduce(
    (acc, t) => {
      if (t.type === "income") acc.income += t.amount;
      else acc.expenses += t.amount;
      return acc;
  ...
  const activeExpenseBudgets = state.budgets.filter(
    (b) =>
      b.type === "expense" &&
      new Date(b.appliesFrom) <= state.displayedMonth &&
  ...
  const valueClass =
    "font-heading font-bold text-3xl sm:text-4xl tracking-tight mb-3";
  ...
  return `
    ...
    typeof getPrivacyClass === "function" ? getPrivacyClass() : ""
  }">R$ ${userSummary.balance.toFixed(2)}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="${cardBase} flex flex-col justify-center">
                            ...
                            <p class="font-heading font-bold text-xl sm:text-2xl text-gray-800 dark:text-gray-100 truncate ${
                              typeof getPrivacyClass === "function"
                                ? getPrivacyClass()
                                : ""
                            }">R$ ${summary.income.toFixed(2)}</p>
                            <p class="text-xs text-gray-400 mt-1 font-medium">Você: <span class="text-brand-600 dark:text-brand-400 ${
                              typeof getPrivacyClass === "function"
                                ? getPrivacyClass()
                                : ""
                            }">R$ ${userSummary.income.toFixed(2)}</span></p>
                        </div>
                        <div class="${cardBase} flex flex-col justify-center">
                            ...
                            <p class="font-heading font-bold text-xl sm:text-2xl text-gray-800 dark:text-gray-100 truncate ${
                              typeof getPrivacyClass === "function"
                                ? getPrivacyClass()
                                : ""
                            }">R$ ${summary.expenses.toFixed(2)}</p>
                            <p class="text-xs text-gray-400 mt-1 font-medium">Você: <span class="text-red-500 ${
                              typeof getPrivacyClass === "function"
                                ? getPrivacyClass()
                                : ""
                            }">R$ ${userSummary.expenses.toFixed(2)}</span></p>
                        </div>
                    </div>
                </div>
    ...
    Icons.Copy
  } Copiar</button><button id="share-link-button" class="flex-1 sm:flex-none px-6 py-3.5 text-sm font-bold text-white bg-brand-500 hover:bg-brand-600 rounded-xl transition shadow-lg shadow-brand-500/30 flex items-center justify-center gap-2">Compartilhar Link</button></div></div></div></div>
    </div>`;
}
...
export function renderRecordsPage() {
  ...
  const filtered = state.transactions.filter((t) => {
    ...
    if (state.selectedDate && tDate.getDate() !== state.selectedDate)
      return false;
  ...
  const filterBadge =
    activeFiltersCount > 0
      ? `<span class="absolute -top-1 -right-1 bg-brand-600 text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full border-2 border-white dark:border-gray-900 shadow-sm animate-bounce">${activeFiltersCount}</span>`
      : "";
...
export function renderBudgetPage() {
  ...
  const activeBudgets = state.budgets.filter((b) => {
    ...
    if (
      ...
      return false;
    if (endDate && endDate < new Date(currentYear, currentMonth, 1))
      return false;
    return true;
  });
  ...
  const budgetItemsHTML = activeBudgets
    .map((budget) => {
      let progressHTML = "";
      let spent = 0;
      ...
      if (budget.type === "expense") {
        ...
        progressHTML = `
                ...
                <div class="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-4 overflow-hidden shadow-inner">
                    <div class="${barColor} h-full rounded-full transition-all duration-500 ease-out relative" style="width: ${Math.min(
          percentage,
          100
        )}%">
                        ${
        ...
        progressHTML = `
                ...
                <div class="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-4 overflow-hidden shadow-inner">
                    <div class="bg-blue-500 h-full rounded-full transition-all duration-500 ease-out" style="width: ${Math.min(
                      percentage,
                      100
                    )}%"></div>
                </div>`;
      ...
      const clickClass = isAdmin
        ? "budget-item cursor-pointer hover:scale-[1.01] hover:shadow-md hover:border-brand-200 dark:hover:border-gray-600"
  ...
  return `
        <div id="budget-page-container" class="pb-32 animate-fade-in">
            ...
            <div class="flex flex-col items-center justify-center py-20 text-gray-400 animate-fade-in">
                ...
                ${
                  isAdmin
                    ? '<p class="text-sm mt-1 opacity-70">Clique em "+ Novo" para criar uma meta de gastos.</p>'
                    : ""
                }
            </div>`
                : ""
...
export function renderDebtsPage() {
  ...
  const totalInstallmentsValue = state.installments.reduce(
  ...
  const getCardStyle = (canEdit) => {
    const base =
      "bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 transition-all relative overflow-hidden group";
    return canEdit
      ? `${base} cursor-pointer hover:shadow-md hover:-translate-y-0.5`
      : base;
  };
  ...
  const debtsHTML =
    state.debts
      .map((debt) => {
        ...
        const realPaid = state.transactions
        ...
        return `
        <div class="${getCardStyle(canEdit)} ${
          canEdit
            ? "debt-item hover:border-red-200 dark:hover:border-red-900"
            : ""
            ...
            <div class="flex items-start justify-between mb-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h4 class="font-heading font-bold text-xl text-gray-900 dark:text-white mb-1">${
                          debt.name
                        }</h4>
            ...
            <div class="space-y-2">
                <div class="flex justify-between text-sm font-bold">
                    <span class="text-gray-500 dark:text-gray-400">Pago: <span class="text-gray-900 dark:text-white">R$ ${realPaid.toFixed(
                      2
                    )}</span></span>
                ...
                <div class="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-3 overflow-hidden">
                    <div class="bg-red-500 h-full rounded-full transition-all duration-500" style="width: ${Math.min(
                      percentage,
                      100
                    )}%"></div>
                </div>
                ${
                  debt.dueDate
                    ? `<p class="text-xs text-right text-gray-400 font-medium mt-2">Vence: ${new Date(
                        debt.dueDate
      ...
      .join("") ||
    `<div class="col-span-full py-12 text-center text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded-3xl border border-dashed border-gray-200 dark:border-gray-700"><p>Nenhuma dívida cadastrada.</p></div>`;

  // --- 2. PARCELAMENTOS (VERDE/BRAND) ---
  const installmentsHTML =
    state.installments
      .map((inst) => {
        const linkedTrans = state.transactions.filter(
          (t) => t.linkedInstallmentId === inst.id
        );
        const paidCount = linkedTrans.length;
        const total = inst.totalValue || 0;
        const percentage =
          inst.installmentsCount > 0
            ? (paidCount / inst.installmentsCount) * 100
            : 0;
        const debtorName =
          state.familyMembers
            .find((m) => m.uid === inst.debtorId)
            ?.name.split(" ")[0] || "???";
        ...
        if (paidCount > 0) {
          ...
          lastPaidText = lastDate.toLocaleString("pt-BR", {
            month: "short",
            year: "numeric",
          });
        }
        ...
        <div class="${getCardStyle(canEdit)} ${
          canEdit
            ? "installment-item hover:border-brand-200 dark:hover:border-brand-900"
            : ""
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                <div class="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-2xl text-center border border-gray-100 dark:border-gray-700">
                    ...
                    <p class="font-heading font-bold text-lg text-brand-600 dark:text-brand-400">${paidCount}<span class="text-gray-400 text-sm">/${
          inst.installmentsCount
        }</span></p>
                </div>
                <div class="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-2xl text-center border border-gray-100 dark:border-gray-700">
                    <p class="text-[10px] font-bold text-gray-400 uppercase">Valor/Mês</p>
                    <p class="font-heading font-bold text-lg text-gray-800 dark:text-white">R$ ${inst.valuePerInstallment.toFixed(
                      0
                    )}</p>
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <div class="flex justify-between items-center pt-2 border-t border-gray-50 dark:border-gray-700/50 mt-2">
                    ...
                    <p class="text-[10px] font-bold text-gray-400 uppercase">Vence dia: <span class="text-gray-600 dark:text-gray-300">${
                      inst.dueDay
                    }</span></p>
                </div>
            </div>
      ...
      .join("") ||
    `<div class="col-span-full py-12 text-center text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded-3xl border border-dashed border-gray-200 dark:border-gray-700"><p>Nenhum parcelamento ativo.</p></div>`;
        ...
        <div class="grid md:grid-cols-2 gap-8">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-center px-2 mb-2">
                    ...
                    ${
                      isAdmin
                        ? `<button id="add-debt-btn" class="text-sm font-bold text-red-600 hover:text-red-700 bg-red-50 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400 px-4 py-2 rounded-xl transition">+ Adicionar</button>`
                        : ""
                    }
                </div>
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-center px-2 mb-2">
                    <div class="flex items-center gap-3">
                        ...
                        ${
                          totalInstallmentsValue > 0
                            ? `<span class="text-xs font-bold text-brand-700 dark:text-brand-300 bg-brand-50 dark:bg-brand-900/30 px-2.5 py-1 rounded-lg border border-brand-100 dark:border-brand-800">Total: R$ ${totalInstallmentsValue.toFixed(
                                2
                              )}</span>`
                            : ""
                        }
                    ...
                    ${
                      isAdmin
                        ? `<button id="add-installment-btn" class="text-sm font-bold text-brand-600 hover:text-brand-700 bg-brand-50 hover:bg-brand-100 dark:bg-brand-900/20 dark:text-brand-400 px-4 py-2 rounded-xl transition">+ Adicionar</button>`
                        : ""
                    }
                </div>
                <div class="grid gap-4">
  ...
  const goalsHTML =
    state.goals
      .map((g) => {
        ...
        return `
        <div class="${interactionClasses} bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 group relative overflow-hidden" data-goal-id="${
          g.id
        }">
            ...
            <div class="flex justify-between items-start mb-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h4 class="font-heading font-bold text-xl text-gray-900 dark:text-white">${
                          g.name
                        }</h4>
                    </div>
                </div>
                ${
                  isCompleted
                    ? '<span class="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 text-xs font-bold px-2 py-1 rounded-lg">Concluída!</span>'
                    : ""
                }
            </div>
            ...
            <div class="space-y-1">
                <div class="flex justify-between items-end">
                    <p class="text-3xl font-heading font-bold ${statusColor}">R$ ${saved.toFixed(
          0
        )}</p>
                    <p class="text-sm text-gray-400 font-medium mb-1">de R$ ${g.targetAmount.toFixed(
                      0
                    )}</p>
                </div>
                <div class="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-3 overflow-hidden">
                ...
                <div class="flex justify-between mt-2">
                    <p class="text-xs text-gray-400 font-bold">${percentage.toFixed(
                      0
                    )}%</p>
                    ${
                      g.deadline
                        ? `<p class="text-xs text-gray-400">Alvo: ${new Date(
                            g.deadline
                          ).toLocaleDateString("pt-BR")}</p>`
                        : ""
                    }
      ...
      .join("") ||
    '<div class="col-span-full py-16 text-center text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded-3xl border border-dashed border-gray-200 dark:border-gray-700"><p class="text-lg font-medium">Nenhuma meta criada.</p></div>';
...
const inputClass =
  "w-full px-4 py-3.5 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-brand-500 focus:border-transparent transition dark:text-white font-medium outline-none";
const selectClass =
  "w-full px-4 py-3.5 bg-white dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-brand-500 focus:border-transparent transition dark:text-white font-medium outline-none appearance-none cursor-pointer";
const labelClass =
  "block text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1.5 ml-1";

export function renderTransactionModal() {
  const isEditing = state.editingTransactionId !== null;
  if (
    !state.isModalOpen ||
    (state.modalView !== "transaction" && state.modalView !== "newTag")
  )
    return "";
  ...
  if (type === "expense") {
    ...
    state.installments.forEach((i) => {
      const selected =
        transaction?.linkedInstallmentId === i.id ? "selected" : "";
  ...
  const inputClass =
    "w-full px-4 py-3.5 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-brand-500 focus:border-transparent transition dark:text-white dark:placeholder-gray-400 font-medium outline-none";
  const selectClass =
    "w-full px-4 py-3.5 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-brand-500 focus:border-transparent transition dark:text-white font-medium outline-none appearance-none cursor-pointer";
  const labelClass =
    "block text-xs font-bold text-gray-400 dark:text-gray-400 uppercase tracking-wider mb-1.5 ml-1";

  let contentHTML = "";
  if (state.modalView === "newTag") {
    const colorSwatches = PALETTE_COLORS.map(
      (color) =>
        `<label class="cursor-pointer hover:scale-110 transition"><input type="radio" name="newTagColor" value="${color}" class="sr-only peer"><div class="w-8 h-8 rounded-full peer-checked:ring-2 ring-offset-2 ring-blue-500" style="background-color: ${color};"></div></label>`
    ).join("");
    ...
    const categoryEmojiOptions = categoryEmojis
      .map(
        (e) =>
          `<label class="cursor-pointer group"><input type="radio" name="newTagIcon" value="${e}" class="sr-only peer"><div class="w-10 h-10 flex items-center justify-center text-xl rounded-xl border-2 border-transparent bg-gray-50 dark:bg-gray-600 peer-checked:border-brand-500 peer-checked:bg-brand-50 dark:peer-checked:bg-brand-900/30 hover:bg-gray-100 dark:hover:bg-gray-500 transition">${e}</div></label>`
      )
      .join("");
    ...
    let categoryOptions = `<option value="" disabled ${
    ...
    const typeSelectorHTML = `
            <div class="flex gap-1 mb-6 p-1 bg-gray-100 dark:bg-gray-700 rounded-xl">
                <button type="button" data-type="expense" class="transaction-type-button flex-1 py-2.5 rounded-lg font-bold text-sm transition-all ${
                  type === "expense"
                    ? "bg-white dark:bg-gray-600 text-red-500 shadow-sm"
                    : "text-gray-500 hover:text-gray-700 dark:text-gray-400"
                }">Despesa</button>
                <button type="button" data-type="income" class="transaction-type-button flex-1 py-2.5 rounded-lg font-bold text-sm transition-all ${
                  type === "income"
    ...
    contentHTML = `
            <form id="${isEditing ? "edit" : "add"}-transaction-form">
                ${typeSelectorHTML}
                <div class="space-y-5">
                    <div><label class="${labelClass}">Descrição</label><input name="description" type="text" class="${inputClass}" value="${
      transaction?.description || ""
    }" required placeholder="Ex: Supermercado" /></div>
                    <div class="grid grid-cols-2 gap-4">
                        <div><label class="${labelClass}">Categoria</label><select id="category" name="category" class="${selectClass}">${categoryOptions}${
      isAdmin
        ? `<option value="--create-new--" class="font-bold text-brand-600 bg-brand-50 dark:bg-brand-900/20">+ ✨ Criar Nova</option>`
        : ""
    }</select></div>
                        <div><label class="${labelClass}">Valor</label><input name="amount" type="number" step="0.01" class="${inputClass}" value="${
      transaction?.amount || ""
    }" required placeholder="0,00" /></div>
                    </div>
                    <div><label class="${labelClass}">Data</label><input name="date" type="date" class="${inputClass}" value="${
      transaction?.date || new Date().toISOString().slice(0, 10)
    }" required /></div>
                    ${
                      type === "expense"
                        ? `<div><label class="${labelClass}">Vincular (Opcional)</label><select name="linkedEntity" class="${selectClass}">${linkOptions}</select></div>`
                        : ""
                    }
                ...
                <div class="mt-8 flex gap-3">
                    ${
                      isEditing
                        ? `<button type="button" id="delete-transaction-button" class="px-4 py-3.5 rounded-xl font-bold text-red-500 bg-red-50 hover:bg-red-100 dark:bg-red-900/10 dark:hover:bg-red-900/30 transition">Excluir</button>`
                        : ""
                    }
...
export function renderBudgetModal() {
  ...
  const budget = isEditing
    ? state.budgets.find((b) => b.id === state.editingBudgetItemId)
    : null;
  ...
  const contentHTML = `
        <form id="budget-form" class="space-y-5">
            <div><label class="${labelClass}">Nome do Orçamento</label><input id="budgetName" name="budgetName" type="text" class="${inputClass}" value="${
    budget?.name || ""
  }" placeholder="Ex: Limite Mercado" required /></div>
            ...
            <div><label class="${labelClass}">Valor Mensal (R$)</label><input id="budgetValue" name="budgetValue" type="number" step="0.01" class="${inputClass}" value="${
    budget?.value || ""
  }" required /></div>
            ...
            <div class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-900/30 rounded-xl border border-gray-100 dark:border-gray-700">
                <input id="budgetRecurring" name="budgetRecurring" type="checkbox" class="w-5 h-5 text-brand-600 rounded focus:ring-brand-500 border-gray-300" ${
                  budget?.recurring ?? true ? "checked" : ""
                }>
            ...
            <div class="mt-8 flex gap-3">
                ${deleteButtonHTML}
                <button type="submit" class="flex-1 py-3.5 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl shadow-lg shadow-brand-500/20 transition">${
...
export function renderDebtModal() {
  ...
  const d = state.editingDebtId
    ? state.debts.find((i) => i.id === state.editingDebtId)
    : null;
  const isAdmin = state.familyAdmins.includes(state.user.uid);
  ...
  if (isAdmin) {
    ...
    const membersOptions = state.familyMembers
      .map(
        (m) =>
          `<option value="${m.uid}" ${
            d?.debtorId === m.uid ? "selected" : ""
          }>${m.name || "Anônimo"}</option>`
      )
      .join("");
  ...
  const contentHTML = `
        <form id="debt-form" class="space-y-5">
            <div><label class="${labelClass}">Nome da Dívida</label><input name="debtName" type="text" class="${inputClass}" value="${
    d?.name || ""
  }" placeholder="Ex: Empréstimo do João" required /></div>
            
            ${debtorFieldHTML}
            
            <div><label class="${labelClass}">Valor Total (R$)</label><input name="debtTotalValue" type="number" step="0.01" class="${inputClass}" value="${
    d?.totalValue || ""
  }" required /></div>
            <div><label class="${labelClass}">Vencimento (Opcional)</label><input name="debtDueDate" type="date" class="${inputClass}" value="${
    d?.dueDate || ""
  }" /></div>
            <div class="mt-8 flex gap-3">
                ${
                  d
                    ? `<button type="button" id="delete-debt-modal-btn" class="px-4 py-3.5 rounded-xl font-bold text-red-500 bg-red-50 hover:bg-red-100 transition">Excluir</button>`
                    : ""
                }
...
export function renderInstallmentModal() {
  ...
  const i = state.editingInstallmentId
    ? state.installments.find((x) => x.id === state.editingInstallmentId)
    : null;
  const membersOptions = state.familyMembers
    .map(
      (m) =>
        `<option value="${m.uid}" ${i?.debtorId === m.uid ? "selected" : ""}>${
          m.name || "Anônimo"
        }</option>`
    )
    .join("");
  ...
  const contentHTML = `
        <form id="installment-form" class="space-y-5">
            <div><label class="${labelClass}">Nome</label><input name="installmentName" type="text" class="${inputClass}" value="${
    i?.name || ""
  }" placeholder="Ex: TV Nova" required /></div>
            <div><label class="${labelClass}">Quem paga?</label><select name="debtorId" class="${selectClass}">${membersOptions}</select></div>
            <div class="grid grid-cols-2 gap-4">
                <div><label class="${labelClass}">Qtd. Parcelas</label><input name="installmentsCount" type="number" class="${inputClass}" value="${
    i?.installmentsCount || ""
  }" required /></div>
                <div><label class="${labelClass}">Valor Mensal</label><input name="valuePerInstallment" type="number" step="0.01" class="${inputClass}" value="${
    i?.valuePerInstallment || ""
  }" required /></div>
            </div>
            <div><label class="${labelClass}">Dia de Vencimento</label><input name="dueDay" type="number" min="1" max="31" class="${inputClass}" value="${
    i?.dueDay || ""
  }" placeholder="Ex: 10" required /></div>
            <div class="mt-8 flex gap-3">
                ${
                  i
                    ? `<button type="button" id="delete-installment-modal-btn" class="px-4 py-3.5 rounded-xl font-bold text-red-500 bg-red-50 hover:bg-red-100 transition">Excluir</button>`
                    : ""
                }
  ...
  return ModalBase(
    i ? "Editar Parcelamento" : "Novo Parcelamento",
    contentHTML
  );
}
...
export function renderFilterModal() {
  ...
  let calendarDaysHTML = Array(firstDay)
    .fill(`<div class="p-2"></div>`)
    .join("");
  for (let day = 1; day <= daysInMonth; day++) {
    const isSelected = T.date === day;
    const hasData = daysWithData.has(day);
    let btnClass =
      "w-8 h-8 rounded-lg text-sm flex items-center justify-center mx-auto transition ";
    if (isSelected)
      btnClass += "bg-brand-600 text-white font-bold shadow-md scale-110";
    else if (hasData)
      btnClass +=
        "bg-brand-100 text-brand-800 dark:bg-brand-900/30 dark:text-brand-300 font-medium";
    else
      btnClass +=
  ...
  const categoriesHTML = uniqueCategories
    .map((cat) => {
      const isActive = T.category === cat;
      const icon = state.categoryIcons[cat] || "🏷️";
      const activeClass = isActive
        ? "bg-brand-600 text-white border-brand-600 shadow-md"
        : "bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-gray-600 hover:border-brand-500";
  ...
  const membersHTML = state.familyMembers
    .map((m) => {
      const isActive = T.member === m.uid;
      const activeClass = isActive
        ? "ring-2 ring-brand-500 ring-offset-2 dark:ring-offset-gray-800 grayscale-0 opacity-100 scale-110"
        : "grayscale opacity-60 hover:opacity-100 hover:grayscale-0";
      let avatar = `<div class="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center font-bold text-gray-500 shadow-sm">${m.name.charAt(
        0
      )}</div>`;
      if (m.photoURL && m.photoURL.includes("|")) {
        const [emoji, bg] = m.photoURL.split("|");
        avatar = `<div class="w-10 h-10 rounded-full flex items-center justify-center text-xl shadow-sm" style="background-color: ${bg}">${emoji}</div>`;
      } else if (m.photoURL) {
        avatar = `<img src="${m.photoURL}" class="w-10 h-10 rounded-full object-cover shadow-sm">`;
      }
      return `<button data-uid="${
        m.uid
      }" class="filter-member-btn flex flex-col items-center gap-1 transition-all ${activeClass} rounded-full p-1">${avatar}<span class="text-[10px] font-bold text-gray-600 dark:text-gray-400">${
        m.name.split(" ")[0]
      }</span></button>`;
    })
    .join("");
  ...
  const typesHTML = typeOptions
    .map((t) => {
      const isActive = T.type === t.val;
      const activeClass = isActive
  ...
  const contentHTML = `
        <div class="space-y-6">
            ...
            <div><label class="${labelClass}">Data (${state.displayedMonth.toLocaleDateString(
    "pt-BR",
    { month: "long", year: "numeric" }
  )})</label><div class="bg-gray-50/50 dark:bg-gray-900/30 rounded-2xl p-3 border border-gray-100 dark:border-gray-700"><div class="grid grid-cols-7 gap-1 text-center mb-2">${[
    "D",
    "S",
    "T",
    "Q",
    ...
    .map(
      (d) =>
        `<span class="text-[10px] font-bold text-gray-300 dark:text-gray-600">${d}</span>`
    )
    .join(
      ""
    )}</div><div class="grid grid-cols-7 gap-1">${calendarDaysHTML}</div></div></div>
...
export function renderManageCategoriesModal() {
  ...
  const categoryListHTML = currentCategories
    .map((cat) => {
  ...
  const typeSelectorHTML = `
        <div class="flex gap-1 mb-6 p-1 bg-gray-100 dark:bg-gray-700/50 rounded-xl">
            <button data-type="expense" class="transaction-type-button flex-1 py-2.5 rounded-lg font-bold text-sm transition-all ${
              currentType === "expense"
                ? "bg-white dark:bg-gray-800 text-red-500 shadow-sm"
                : "text-gray-500 hover:text-gray-700 dark:text-gray-400"
            }">Despesa</button>
            <button data-type="income" class="transaction-type-button flex-1 py-2.5 rounded-lg font-bold text-sm transition-all ${
              currentType === "income"
  ...
  const contentHTML = `
        <div>
            ...
            <div class="space-y-2 mb-6 max-h-60 overflow-y-auto custom-scrollbar pr-1">
                <ul class="">
                    ${
                      categoryListHTML.length
                        ? categoryListHTML
                        : '<p class="p-8 text-center text-gray-400 text-sm font-medium bg-gray-50 dark:bg-gray-800/30 rounded-2xl border border-dashed border-gray-200 dark:border-gray-700">Nenhuma categoria personalizada encontrada.</p>'
                    }
                </ul>
            </div>
    ...
    .map(
      (e) =>
        `<label class="cursor-pointer group">
            <input type="radio" name="editCategoryIcon" value="${e}" class="sr-only peer" ${
          e === currentIcon ? "checked" : ""
        }>
...
export function renderGoalModal() {
  ...
  const g = state.editingGoalId
    ? state.goals.find((x) => x.id === state.editingGoalId)
    : null;

  // Botão de Excluir (só aparece se estiver editando)
  const deleteBtn = state.editingGoalId
    ? `<button type="button" id="delete-goal-modal-btn" class="px-4 py-3.5 rounded-xl font-bold text-red-500 bg-red-50 hover:bg-red-100 dark:bg-red-900/10 dark:hover:bg-red-900/30 transition">Excluir</button>`
    : "";
  ...
  const emojiOptions = goalEmojis
    .map(
      (e) =>
        `<label class="cursor-pointer group"><input type="radio" name="goalIcon" value="${e}" class="sr-only peer" ${
          e === currentIcon ? "checked" : ""
        }><div class="w-10 h-10 flex items-center justify-center text-xl rounded-xl border-2 border-transparent bg-gray-50 dark:bg-gray-700 peer-checked:border-brand-500 peer-checked:bg-brand-50 dark:peer-checked:bg-brand-900/30 hover:scale-110 transition">${e}</div></label>`
    )
    .join("");
  ...
  const colorSwatches = PALETTE_COLORS.map(
    (color) =>
      `<label class="cursor-pointer hover:scale-110 transition"><input type="radio" name="goalColor" value="${color}" class="sr-only peer" ${
        color === currentColor ? "checked" : ""
      }><div class="w-8 h-8 rounded-full peer-checked:ring-2 ring-offset-2 ring-gray-400 dark:ring-gray-600 shadow-sm" style="background-color: ${color};"></div></label>`
  ).join("");
  ...
  const contentHTML = `
        <form id="goal-form" class="space-y-5">
            <div><label class="${labelClass}">Nome da Meta</label><input name="goalName" type="text" class="${inputClass}" value="${
    g?.name || ""
  }" placeholder="Ex: Viagem Disney" required /></div>
            <div><label class="${labelClass}">Valor Alvo (R$)</label><input name="targetAmount" type="number" step="0.01" class="${inputClass}" value="${
    g?.targetAmount || ""
  }" required /></div>
            <div><label class="${labelClass}">Data Alvo (Opcional)</label><input name="deadline" type="date" class="${inputClass}" value="${
    g?.deadline || ""
  }" /></div>
...
export function renderSettingsModal() {
  if (!state.user || !state.isModalOpen || state.modalView !== "settings")
    return "";
  ...
  const generateEmojiOptions = (emojiList) =>
    emojiList
      .map(
        (e) =>
          `<label class="cursor-pointer group"><input type="radio" name="avatarEmoji" value="${e}" class="sr-only peer/emoji" ${
            e === currentEmoji ? "checked" : ""
          }><div class="w-10 h-10 flex items-center justify-center text-xl rounded-xl border-2 border-transparent bg-gray-50 dark:bg-gray-700 peer-checked:border-brand-500 peer-checked:bg-brand-50 dark:peer-checked:bg-brand-900/30 hover:scale-110 transition shadow-sm">${e}</div></label>`
      )
      .join("");
...
export function renderFamilyInfoModal() {
  ...
  const memberListHTML = state.familyMembers
    .map((member) => {
      ...
      let avatarHTML = `<div class="w-10 h-10 rounded-full bg-brand-100 dark:bg-gray-700 flex items-center justify-center text-brand-600 dark:text-gray-300 font-heading font-bold text-lg mr-3 select-none border-2 border-white dark:border-gray-600 shadow-sm">${finalName
        .charAt(0)
        .toUpperCase()}</div>`;
      if (finalPhoto) {
      ...
      const balanceColor =
        memberBalance >= 0
          ? "text-brand-600 dark:text-brand-400"
          : "text-red-500 dark:text-red-400";
      const adminTag = isMemberAdmin
        ? `<span class="ml-2 bg-brand-100 text-brand-700 text-[10px] font-bold px-2 py-0.5 rounded-full dark:bg-brand-900/30 dark:text-brand-300 border border-brand-200 dark:border-brand-800">Admin</span>`
        : "";
      ...
      return `<li class="flex items-center justify-between p-4 border-b border-gray-100 last:border-b-0 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors"><div class="flex items-center">${avatarHTML}<div><div class="flex items-center gap-1"><p class="font-heading font-bold text-gray-800 dark:text-gray-200 text-sm">${displayName}</p>${adminTag}</div><p class="text-xs font-medium ${balanceColor} mt-0.5">R$ ${memberBalance.toFixed(
        2
      )}</p></div></div><div class="flex items-center gap-1">${actionButtons}</div></li>`;
    })
    .join("");
  ...
  const editNameButton = isAdmin
    ...
    : "";
  const regenerateCodeButton = isAdmin
    ? `<button id="regenerate-code-btn" class="p-2 text-gray-400 hover:text-brand-600 transition" title="Gerar novo código"><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg></button>`
    : "";
  const deleteFamilyButton = isAdmin
    ? `<button id="delete-family-button" class="w-full py-3.5 px-4 rounded-xl font-bold text-red-600 bg-red-50 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/30 transition mt-2 border border-red-100 dark:border-red-900/50">Excluir Família</button>`
    : "";
...
function renderBudgetPerformanceChart() {
  ...
  const activeBudgets = state.budgets.filter(
    (b) =>
      b.type === "expense" && new Date(b.appliesFrom) <= state.displayedMonth
  );
    ...
    const spent = state.transactions
      .filter(
        (t) =>
          ...
          new Date(t.date + "T12:00:00").getMonth() ===
  ...
  return new Chart(canvas.getContext("2d"), {
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: true,
          grid: { color: state.theme === "dark" ? "#374151" : "#e2e8f0" },
...
export function renderPersonSpendingChart() {
  ...
  const noDataElement = document.getElementById(
    "person-spending-chart-no-data"
  );
  ...
  state.familyMembers.forEach((m) => {
    ...
    if (m.photoURL && m.photoURL.includes("|")) {
      ...
      image = new Image();
      image.src = m.photoURL;
      image.crossOrigin = "Anonymous";
      image.onload = () => {
        const chart = Chart.getChart("person-spending-chart");
  ...
  const gridColor = state.theme === "dark" ? "#374151" : "#e2e8f0";
    ...
    afterDraw: (chart) => {
      ...
      x.ticks.forEach((tick, index) => {
        ...
        if (
          ...
          if (user.icon.length === 1 && user.icon.match(/[a-z]/i)) {
        ...
        const balanceFormatted = "R$ " + user.balance.toFixed(0);
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            title: (context) =>
              context[0].chart.data.userData[context[0].dataIndex].name,
          },
        },
        datalabels: { display: false },
      },
...
function renderAnnualChart() {
  const canvas = document.getElementById("annual-balance-chart");
  ...
  state.transactions.forEach((t) => {
    ...
    if (d.getFullYear() === year) {
      if (t.type === "income") income[d.getMonth()] += t.amount;
      else expense[d.getMonth()] += t.amount;
    }
  });
  const textColor = state.theme === "dark" ? "#e2e8f0" : "#475569";
...
function renderComparisonChart() {
  ...
  if (!canvas) return null;
  const curr = state.displayedMonth;
  ...
  const getExp = (d) =>
    state.transactions
      .filter(
        (t) =>
          t.type === "expense" &&
          new Date(t.date + "T12:00:00").getMonth() === d.getMonth() &&
          new Date(t.date + "T12:00:00").getFullYear() === d.getFullYear()
      )
      .reduce((s, t) => s + t.amount, 0);
  const textColor = state.theme === "dark" ? "#e2e8f0" : "#475569";
  ...
  return new Chart(canvas.getContext("2d"), {
    ...
    data: {
      labels: [
        prev.toLocaleString("pt-BR", { month: "short" }),
        curr.toLocaleString("pt-BR", { month: "short" }),
      ],
      datasets: [
        {
          label: "Gastos",
          data: [getExp(prev), getExp(curr)],
...
function renderDailyEvolutionChart() {
  const canvas = document.getElementById("daily-evolution-chart");
  if (!canvas) return null;
  const days = new Date(
  ...
  const data = labels.map((d) => {
    const daily = state.transactions
      .filter(
        (t) =>
          t.type === "expense" &&
          new Date(t.date + "T12:00:00").getDate() === d &&
          new Date(t.date + "T12:00:00").getMonth() ===
            state.displayedMonth.getMonth()
      )
      .reduce((s, t) => s + t.amount, 0);
    ...
    return new Date().getDate() < d &&
      new Date().getMonth() === state.displayedMonth.getMonth()
      ? null
      : acc;
  });
  const textColor = state.theme === "dark" ? "#e2e8f0" : "#475569";
...
export function renderExportModal() {
  ...
  const buttonClass =
    "w-full flex items-center justify-between p-4 rounded-2xl border-2 border-gray-100 dark:border-gray-700 hover:border-brand-500 dark:hover:border-brand-500 bg-white dark:bg-gray-800 hover:bg-brand-50 dark:hover:bg-brand-900/20 transition-all group cursor-pointer shadow-sm";
  const textClass =
    "font-heading font-bold text-gray-800 dark:text-gray-100 text-lg group-hover:text-brand-600 dark:group-hover:text-brand-400";
